...
The Bleve implementation is also fast but is still pretty fiddly to get set up because of the dependency on `libfaiss` (see details below). It does not require loading all data in to memory but the index files are very large (pending some optimizations) and can be quite slow to finish writing to disk.
