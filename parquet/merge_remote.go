...
// MergeRemote [*embeddingsdb.Record] records stored in a remote Parquet file identified by 'uri' and add them to 'wr'
func MergeRemote(ctx context.Context, wr *ParquetWriter, uri *url.URL) (int64, error) {
	...
	go func() {
		...
		for {
			select {
			case <-done_ch:
				logger.Debug("Records merged", "count", count)
				return
			case <-ticker.C:
				logger.Debug("Records merged", "count", count)
			}
	...
	for rows.Next() {
		...
		n, err := wr.Write([]*embeddingsdb.Record{row})
